...
-- 2) Set defaults to allow anonymous submissions for now
ALTER TABLE public.service_requests
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='service_requests' AND policyname='Public can create service requests'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='service_requests' AND policyname='Public can view recent service requests'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='service_communications' AND policyname='Public can create service communications'
  ) THEN
