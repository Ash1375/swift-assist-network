    ...
    <div className="w-full max-w-7xl mx-auto p-4 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <Input
                ...
                className="pl-12 h-12 text-base border-2 border-border/50 focus:border-primary rounded-xl"
              />
            </div>
            <div className="flex gap-3 flex-wrap">
              <Button
                ...
                className="gap-2 h-12 px-6 rounded-xl shadow-sm"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="shadow-xl border-0 bg-gradient-to-br from-card to-card/80 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="h-[450px] rounded-xl bg-gradient-to-br from-blue-50 via-emerald-50 to-orange-50 relative overflow-hidden shadow-inner border">
              ...
              {userLocation && (
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
                  <div className="relative">
                    <div className="w-5 h-5 bg-gradient-to-r from-blue-600 to-blue-700 rounded-full border-3 border-white shadow-lg" />
                    <div className="absolute -top-2 -left-2 w-9 h-9 bg-blue-600/20 rounded-full animate-ping" />
                  </div>
                  <span className="absolute top-8 left-1/2 transform -translate-x-1/2 text-xs font-semibold bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full shadow-lg border">
                    You are here
                  </span>
                </div>
              )}
              ...
              {filteredStations.slice(0, 6).map((station, index) => (
                ...
                >
                  <div className={`w-10 h-10 bg-gradient-to-r ${getStationGradient(station.type)} rounded-full flex items-center justify-center text-white shadow-lg border-3 border-white hover:shadow-xl transition-shadow`}>
                    {getStationIcon(station.type)}
                  </div>
                  {selectedStation?.id === station.id && (
                    <div className="absolute top-12 left-1/2 transform -translate-x-1/2 bg-white/95 backdrop-blur-sm p-3 rounded-xl shadow-xl border min-w-[220px] animate-scale-in">
              ...
              <Button
                size="sm"
                className="absolute top-4 right-4 shadow-lg rounded-xl bg-white/90 backdrop-blur-sm border text-foreground hover:bg-white"
                onClick={() => {
          ...
          <CardContent className="p-0">
            <div className="max-h-[450px] overflow-y-auto">
              {filteredStations.map((station) => (
                <div
                  key={station.id}
                  className={`p-5 border-b last:border-b-0 cursor-pointer transition-all duration-200 hover:bg-gradient-to-r hover:from-primary/5 hover:to-transparent ${
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-4 flex-1">
                      <div className={`w-12 h-12 bg-gradient-to-r ${getStationGradient(station.type)} rounded-xl flex items-center justify-center text-white shadow-lg`}>
                        {getStationIcon(station.type)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          <h4 className="font-semibold text-base truncate">{station.name}</h4>
                          <Badge 
                            variant={station.isOpen ? 'default' : 'secondary'} 
                            className={`text-xs px-2 py-1 ${station.isOpen ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}
                          >
                        ...
                        <div className="flex items-center gap-6 text-sm">
                          ...
                          <span className="text-muted-foreground font-medium">{getTypeLabel(station.type)}</span>
                        </div>
                        {station.amenities && (
                          <div className="flex gap-2 mt-3 flex-wrap">
                            {station.amenities.slice(0, 3).map((amenity) => (
                              <Badge key={amenity} variant="outline" className="text-xs px-2 py-1 bg-white/50">
                                {amenity}
                              </Badge>
                            ))}
                          </div>
      ...
      {selectedStation && (
        <Card className="shadow-xl border-0 bg-gradient-to-br from-card to-card/80 backdrop-blur-sm">
          <CardHeader className="pb-4">
            <CardTitle className="flex items-center gap-3 text-xl">
              <div className={`p-2 rounded-lg bg-gradient-to-r ${getStationGradient(selectedStation.type)}`}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3">
                  ...
                  <Badge 
                    ...
                    className={`${selectedStation.isOpen ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`}
                  >
                    {selectedStation.isOpen ? 'Open Now' : 'Closed'}
                  </Badge>
                </div>
                <p className="text-sm text-muted-foreground mt-1">{getTypeLabel(selectedStation.type)}</p>
              </div>
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-muted/30 rounded-xl p-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {selectedStation.amenities.map((amenity) => (
                        <Badge key={amenity} variant="outline" className="bg-white/50">
                          {amenity}
                        </Badge>
                      ))}
                    </div>
              ...
              <div className="space-y-4">
                <div className="bg-primary/5 rounded-xl p-4 border border-primary/20">
                  <h4 className="font-semibold mb-3 text-primary">Connect Through Our Platform</h4>
